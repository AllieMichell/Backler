...
def getName():
    ...
    nameP = str.split(projectName, ' ')
    nameProjectC = '%20'.join(nameP)
    return nameProjectC
nameProject = getName()


ENV = os.environ["IP_API"]
uri = "http://{0}/Backler/api/projects/nameP/{1}".format(ENV, nameProject)
print(uri)
    ...
    try:
        ...
        if dbtype == "mysql":
            ...
            if os.path.exists("latest.sql"):
                ...
                module.mysqlDump_SSH(servername, serverhost, serverport, serverpassword, ssh, sshpath,
                                     remotepath, localpath, dbip, dbtype, dbuser, dbpassword, dbname, dbtimestamp, projectName)
            else:
                module.mysqlDump_SSH(servername, serverhost, serverport, serverpassword, ssh, sshpath,
                                     remotepath, localpath, dbip, dbtype, dbuser, dbpassword, dbname, dbtimestamp, projectName)
        ...
        if dbtype == "mongodb":
            ...
            if os.path.exists("latest"):
                ...
                module.mongoDump_SSH(servername, serverhost, serverport, serverpassword, ssh, sshpath,
                                     remotepath, localpath, dbip, dbtype, dbport, dbname, dbtimestamp, projectName)
            else:
                module.mongoDump_SSH(servername, serverhost, serverport, serverpassword, ssh, sshpath,
                                     remotepath, localpath, dbip, dbtype, dbport, dbname, dbtimestamp, projectName)
        ...
        if dbtype == "jenkins":
            ...
            if os.path.exists("latest.zip"):
                ...
                module.jenkins_SSH(servername, serverhost, serverport, serverpassword, ssh,
                                   sshpath, remotepath, localpath, dbtype, dbname, dbtimestamp, projectName)
            else:
                module.jenkins_SSH(servername, serverhost, serverport, serverpassword, ssh,
                                   sshpath, remotepath, localpath, dbtype, dbname, dbtimestamp, projectName)
...
#TELNET BACKUP
    ...
    if dbtype == "mysql":
        if dbuser == "" or dbpassword == "" or dbip == "" or dbname == "":
            MESSAGE = "Warnin: se encontraron campos vacios para respaldar la base de datos: {0} !".format(
                dbname)
            DB_STATUS = "warning"
            #Connection to Mongodb and save Log
            logs = mongotools.JSON_SCHEMA(
                dbname, dbtype, dbtimestamp, dbip, DB_STATUS, localpath, MESSAGE, ssh)
            ...
            try:
                flag = str(dbtools.testConnectingDB_MYSQL(
                    dbuser, dbpassword, dbip, dbname))

            except Exception as e:
                MESSAGE = "Error en la conexion a la base de datos: {0} !".format(
                    dbname)
                DB_STATUS = "error"
                # Connection to Mongodb and save Log
                logs = mongotools.JSON_SCHEMA(
                ...
                if flag == "1":
                    ...
                    if os.path.exists("latest.sql"):
                        dbtools.renameFile_MySQL("latest.sql")
                        module.mysqlDump_Module(
                            ssh, localpath, dbip, dbtype, dbuser, dbpassword, dbname, dbtimestamp, projectName)
                    else:
                        module.mysqlDump_Module(
                            ssh, localpath, dbip, dbtype, dbuser, dbpassword, dbname, dbtimestamp, projectName)
                else:
    ...
    if dbtype == "mongodb":
        ...
        if os.path.exists("latest"):
            dbtools.renameFile_MongoDB("latest")
            module.mongoDump_Module(
                ssh, localpath, dbip, dbtype, dbport, dbname, dbtimestamp, projectName)
        else:
            module.mongoDump_Module(
                ssh, localpath, dbip, dbtype, dbport, dbname, dbtimestamp, projectName)
